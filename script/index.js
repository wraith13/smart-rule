            ...
            case "logarithmic":
                {
                    ...
                    if (view.viewScale < 100) {
                        ...
                        for (var a = begin; a <= end; a += begin / scale) {
                            var value = a;
                            if (value <= max) {
                                ticks.push({ value: value, type: "long", });
                                for (var b = 0; b < scale; ++b) {
                                    var value_1 = a + (b * (begin / (scale * scale)));
                                    if (value_1 <= max) {
                                        ticks.push({
                                            value: value_1,
...
define("script/event", ["require", "exports", "script/type", "script/environment", "script/view", "script/ui", "script/render", "script/ruler", "script/grid", "script/graph"], function (require, exports, Type, Environment, View, UI, Render, Ruler, Grid, Graph) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    exports.initialize = exports.resetZoom = exports.zoomOut = exports.zoomIn = exports.updateScaleModeRoundBar = exports.updateViewModeRoundBar = void 0;
    Type = __importStar(Type);
    ...
    var zoomIn = function () {
        var current = View.data.viewScale;
        var next = current * 1.25;
        View.data.viewScale = next;
        Render.markDirty();
        console.log("Zoomed in: ".concat(current, " -> ").concat(next));
    };
    ...
    var zoomOut = function () {
        ...
        View.data.viewScale = next;
        Render.markDirty();
        console.log("Zoomed out: ".concat(current, " -> ").concat(next));
    };
    ...
    var initialize = function () {
        ...
        document.addEventListener("pointermove", function (event) {
            if ("touch" === event.pointerType) // && event.isPrimary)
             {
                var touches = Array.from(event.getCoalescedEvents()).filter(function (e) { return e.pointerType === "touch"; });
                if (2 <= touches.length) {
